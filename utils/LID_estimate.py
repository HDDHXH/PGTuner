...
def intrinsic_dim(X, method='MLE_NN'):
    """
...
def mle_nn(X):
    ...
    try:
        ...
        nn = NearestNeighbors(n_neighbors=k2+1, algorithm='brute', metric='euclidean')
        nn.fit(X)

        '''
            这里如果数组太大，在计算距离矩阵时会触发cuda out of memory 的异常，按下面的逻辑处理：
            1.判断数组数据量是否超过1000万，如果不超过1000万，则在触发异常时，将数组分5批处理；如果每批数组再触发异常，
            则架构每批数组再分5批处理，这个逻辑只要判断这两次就行，不需要递归。
            2.如果超过1000万，则在触发异常时，将数组分10批处理；如果每批数组再触发异常，则架构每批数组再分10批处理，
            这个逻辑也只要判断这两次就行，不需要递归。
            
        '''
        try:
            distances, _ = nn.kneighbors(X)
            log_distances = cp.log(distances[:, 1:k2 + 1] + 1e-5)  # 从第二列开始，因为第一列是每个点到自身的距离（0）

            del distances
        
        except MemoryError:

            def batch_process(X, n, batch, nn):
                log_distances_ = []
                ...
                for i in range(batch):
                    start = i * batch_size
                    end = start + batch_size if i < batch-1 else n

                    try:
                        temp_distances, _ = nn.kneighbors(X[start:end])
                        temp_log_distances = cp.log(temp_distances[:, 1:k2 + 1] + 1e-5)
                        log_distances_.append(temp_log_distances)

                        del temp_distances
                        del temp_log_distances

                    except MemoryError:
                        sub_batch_size = (end - start) // batch

                        for j in range(batch):
                            sub_start = start + j * sub_batch_size
                            sub_end = sub_start + sub_batch_size if j < batch-1 else end

                            sub_temp_distances, _ = nn.kneighbors(X[sub_start:sub_end])
                            sub_temp_log_distances = cp.log(sub_temp_distances[:, 1:k2 + 1] + 1e-5)
                            log_distances_.append(sub_temp_log_distances)

                        del sub_temp_distances
                        del sub_temp_log_distances

                log_distances = cp.concatenate(log_distances_, axis=0)

                return log_distances

            if n < 1e7:
                log_distances = batch_process(X, n, 5, nn)
            
            else:
                log_distances = batch_process(X, n, 10, nn)

        # 准备 k 值数组
        indexk = cp.arange(k1, k2+1)

        # 计算 S：对于每个 k 值，累积距离的对数
        S = cp.cumsum(log_distances, axis=1)

        # 应用 MLE 公式估计维度
        dhat = cp.mean(-(indexk - 2) / (S[:, k1 - 1:k2] - log_distances[:, k1 - 1:k2] * cp.tile(indexk, (n, 1)) + 1e-5))

        if math.isinf(dhat):
            dhat = 500
        else:
            dhat = math.floor(dhat+0.5)

        del log_distances
        del nn

    except MemoryError: #不行就放到cpu上跑
        nn = NearestNeighbors_CPU(n_neighbors=k2+1, algorithm='brute', metric='euclidean')
        nn.fit(X)

        try:
            distances, _ = nn.kneighbors(X)
            log_distances = np.log(distances[:, 1:k2 + 1] + 1e-5)  # 从第二列开始，因为第一列是每个点到自身的距离（0）
            ...
            def batch_process(X, n, batch, nn):
                ...
                for i in range(batch):
                    ...
                    try:
                        temp_distances, _ = nn.kneighbors(X[start:end])
                        temp_log_distances = np.log(temp_distances[:, 1:k2 + 1] + 1e-5)
                        log_distances_.append(temp_log_distances)
                        ...
                        for j in range(batch):
                            ...
                            sub_temp_distances, _ = nn.kneighbors(X[sub_start:sub_end])
                            sub_temp_log_distances = np.log(sub_temp_distances[:, 1:k2 + 1] + 1e-5)
                            log_distances_.append(sub_temp_log_distances)

                        del sub_temp_distances
                        del sub_temp_log_distances

                log_distances = np.concatenate(log_distances_, axis=0)
        ...
        S = np.cumsum(log_distances, axis=1)
    
        dhat = np.mean(-(indexk - 2) / (S[:, k1 - 1:k2] - log_distances[:, k1 - 1:k2] * np.tile(indexk, (n, 1)) + 1e-5))
