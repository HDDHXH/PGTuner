...
from TD3_nsg import *
from index_env import IndexEnv_new_state_onlys_eval_nsg
from Args import args as args_r
from utils import Logger, time_start
...
if __name__ == '__main__':
    ...
    data_path = os.path.join(parent_directory, 'NSG_KNNG/Data/test_data_main.csv')  # 用于测试的真实数据集，用于测试泛化性能的真实数据集记为real_data_test_generalization
    ...
    for dataset_name in [ 'gist']:  #['glove', 'sift50', 'deep10', 'gist']
        ...
        if dataset_name in ['sift50', 'deep10', 'glove', 'gist']:
            predict_model_save_path = os.path.join(parent_directory, 'query_performance_predict/model_checkpoints/DiPredict/NSG_KNNG/{}_{}_{}_{}_{}_2_6_checkpoint.pth'.format(dataset_name,
                                                            args_p.dipredict_layer_sizes_nsg, args_p.dipredict_n_epochs,
                                                            args_p.dipredict_batch_size, args_p.dipredict_lr))

            standard_path = os.path.join(parent_directory, 'query_performance_predict/scaler_paras/NSG_KNNG/{}_feature_standard_2_6.npz'.format(dataset_name))

        else:
            predict_model_save_path = os.path.join(parent_directory,
                                                   'query_performance_predict/model_checkpoints/DiPredict/NSG_KNNG/{}_{}_{}_{}_checkpoint.pth'.format(
                                                       args_p.dipredict_layer_sizes_nsg, args_p.dipredict_n_epochs,
                                                       args_p.dipredict_batch_size, args_p.dipredict_lr))

            standard_path = os.path.join(parent_directory, 'query_performance_predict/scaler_paras/NSG_KNNG/feature_standard.npz')
        ...
        for cfg in tqdm(cfg_lis, total=len(cfg_lis)):
            ...
            interval_best_paras_file = os.path.join(current_directory + '/' + stor_subdir,
                                           'eval_interval_best_paras_{}_{}_{}_{}.pkl'.format(expr_name, dataset_name, args_r.nochange_episodes, args_r.test_epoches))
            ...
            episode_dec_file = os.path.join(current_directory + '/' + stor_subdir,
                                            'eval_episode_dec_{}_{}_{}_{}.pkl'.format(expr_name, dataset_name, args_r.nochange_episodes, args_r.test_epoches))
            ...
            logger = Logger(name=args_r.method, log_file=stor_subdir + '/' + 'log/eval_{}_{}_{}_{}.log'.format(expr_name, dataset_name, args_r.nochange_episodes, args_r.test_epoches))

            memory_file = os.path.join(current_directory + '/' + stor_subdir, 'save_memory/train_{}.pkl'.format(expr_name))

            # 加载并初始化环境
            print('----------------初始化环境----------------')
            env = IndexEnv_new_state_onlys_eval_nsg(num_dataset, final_default_performance, target_rec_lis, args_r, args_p, predict_model_save_path, standard_path, device)
