...
from utils import Scaler_para_gpu
...
class OUProcess():
    def __init__(self, num_data, size, sigma_decay_rate, n_steps_annealing, sigma, mu=0.0, sigma_min = 0.002, theta=0.3, dt=1e-2, x_prev=None):
        ...
        self.dt = dt
        self.x_prev = x_prev
        self.shape = (num_data, size)  #(num_data, 3)
    ...
    def current_sigma(self):
        # sigma = max(self.sigma_min, self.m * float(self.n_steps) + self.c)  #线性退火
        sigma = max(self.sigma_min, self.sigma * (self.sigma_decay_rate ** self.n_steps))  #指数退火
...
class Actor(nn.Module):
    def __init__(self, n_states, n_actions, actor_layer_sizes, dv):
        ...
        layer_list = []
        layer_sizes = [int(x) for x in actor_layer_sizes]  # actor_layer_sizes是中间层，暂时定为[128, 128, 64]

        layer_list.append(nn.Linear(n_states, layer_sizes[0]))  # 第一层
        layer_list.append(nn.ReLU(inplace=False))

        for i in range(len(layer_sizes) - 1):  # 中间层
...
class Critic(nn.Module):
    def __init__(self, n_states, n_actions, critic_layer_sizes):
        ...
        layer_list = []
        layer_sizes = [int(x) for x in critic_layer_sizes]  # critic_layer_sizes是中间层，暂时定为[256, 256, 64]
...
class TD3(object):
    def __init__(self, n_states, n_actions, num_data, opt, dv):
        ...
        self.para_scaler = Scaler_para_gpu(dv)
        ...
        self.tau = opt['tau']
        # self.policy_noise_clip = opt['policy_noise_clip']
        self.update_time = 0
        ...
        # self.value_noise = Gaussian_noise_value(self.batch_size, n_actions, opt['sigma_value'])
    ...
    def add_sample(self, states, actions, rewards, next_states, terminates):
        ...
        with torch.no_grad():
            ...
            # 将terminate数组转换为discount factor数组
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            # action_noise = self.totensor(self.value_noise.noise()).to(self.device)
            # action_noise = torch.clamp(action_noise, -self.policy_noise_clip / 2, self.policy_noise_clip)
            # target_next_actions = torch.clamp(target_next_actions+action_noise, 0, 1)
    ...
    def choose_action(self, x, train=True):
        ...
        #
        #         temp_x = x[batch_start: batch_end, :]
        #         temp_act = self.actor(temp_x)
        #
        #         action_lis.append(temp_act)
        # act = torch.cat(action_lis, dim=0)
