...
from TD3_nsg import *
from index_env import  IndexEnv_new_state_onlys_nsg
...
if __name__ == '__main__':
    ...
    data_path = os.path.join(parent_directory, 'NSG_KNNG/Data/train_data.csv')  # 这个只包含训练数据集的特征和性能数据
    ...
    for cfg in tqdm(cfg_lis, total=len(cfg_lis)):
        ...
        # 加载并初始化环境
        print('----------------初始化环境----------------')
        env = IndexEnv_new_state_onlys_nsg(num_dataset, final_default_performance, target_rec_lis, args_r, args_p, predict_model_save_path, standard_path, device)
