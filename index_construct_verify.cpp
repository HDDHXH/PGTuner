...
#pragma omp critical
        ...
        int real_efS = std::get<0>(results[0]);     // 获取 ef 值
        float real_rec = std::get<1>(results[0]);   // 获取召回率
        float tpq = std::get<2>(results[0]);        // 获取每查询的时间
        size_t s_dc_counts = std::get<3>(results[0]); // 获取统计计数
        float p_st = std::get<4>(results[0]);        // 获取实际参数的搜索时间
