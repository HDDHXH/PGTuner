...
#pragma omp critical
        ...
        int real_efS1 = std::get<0>(results1[0]);     // 获取 ef 值
        float real_rec1 = std::get<1>(results1[0]);   // 获取召回率
        float tpq1 = std::get<2>(results1[0]);        // 获取每查询的时间
        size_t s_dc_counts1 = std::get<3>(results1[0]); // 获取统计计数
        float p_st1 = std::get<4>(results1[0]);        // 获取实际参数的搜索时间

        int real_efS2 = std::get<0>(results2[0]);     // 获取 ef 值
        float real_rec2 = std::get<1>(results2[0]);   // 获取召回率
        float tpq2 = std::get<2>(results2[0]);        // 获取每查询的时间
        size_t s_dc_counts2 = std::get<3>(results2[0]); // 获取统计计数
        float p_st2 = std::get<4>(results2[0]);        // 获取实际参数的搜索时间

        int real_efS3 = std::get<0>(results3[0]);     // 获取 ef 值
        float real_rec3 = std::get<1>(results3[0]);   // 获取召回率
        float tpq3 = std::get<2>(results3[0]);        // 获取每查询的时间
        size_t s_dc_counts3 = std::get<3>(results3[0]); // 获取统计计数
        float p_st3 = std::get<4>(results3[0]);        // 获取实际参数的搜索时间

        int real_efS4 = std::get<0>(results4[0]);     // 获取 ef 值
        float real_rec4 = std::get<1>(results4[0]);   // 获取召回率
        float tpq4 = std::get<2>(results4[0]);        // 获取每查询的时间
        size_t s_dc_counts4 = std::get<3>(results4[0]); // 获取统计计数
        float p_st4 = std::get<4>(results4[0]);        // 获取实际参数的搜索时间
