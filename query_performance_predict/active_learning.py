...
from utils import read_data, read_data_new, read_unlabeld_data_new, split_data,  df2np, np2ts, CustomDataset, Scaler_minmax, load_model, Scaler_minmax_new_gpu, calculate_errors
from Args import args

'''
用于检测基输入特征向量和用于选择的基输入特征向量都是用的训练数据。这种方式在检测和泛化效果上是最好的，也是最符合直觉的，所以主动学习就用这个
'''
...
if __name__ == '__main__':
    ...
    init_model_save_path = os.path.join(current_directory, 'model_checkpoints/DiPredict/{}_{}_{}_{}_checkpoint.pth'.format(
    ...
    init_test_data_path = os.path.join(parent_directory, 'Data/test_data_main.csv') #用于加载选择的未标注参数组合的标注数据
    ...
    #selected_num = 1
    selected_group_num = 24  #研究selected_num的作用时，需要固定最大的选择的配置组合的数量，然后来比较不同selected_group_num的影响。sift50用18,另外3个用24；这个同时也能体现出在固定selected_num的情况下，selected_rounds的作用
    ...
    for selected_num in [1]:
        for dataset_name in ['glove']:
            print(dataset_name)
            ...
            new_train_data_path = "../Data/active_learning_data/{}_train_data_{}_{}.csv".format(dataset_name, selected_num, selected_rounds)
            selected_config_path = "../Data/active_learning_data/{}_selected_config_{}_{}.csv".format(dataset_name, selected_num, selected_rounds)
            fig_save_path = "../Data/active_learning_data/{}_distance_{}_{}.png".format(dataset_name, selected_num, selected_rounds)

            new_feature_standard_path = os.path.join(current_directory, 'scaler_paras/{}_feature_standard_{}_{}.npz'.format(dataset_name, selected_num, selected_rounds))
            new_model_save_path = os.path.join(current_directory, 'model_checkpoints/DiPredict/{}_{}_{}_{}_{}_{}_{}_checkpoint.pth'.format(dataset_name,
                                                            args.dipredict_layer_sizes, args.dipredict_n_epochs,
            ...
            test_error_path = "../Data/active_learning_data/{}_test_error_{}_{}.csv".format(dataset_name, selected_num, selected_rounds)
            ...
            selected_df_feature_labeled = current_train_df[ ['efConstruction', 'M', 'efSearch', 'SIZE', 'q_SIZE', 'DIM', 'LID', 'Sum_K_MinDist', 'Sum_K_MaxDist',
            ...
            # #组合得到的输入特征向量用于检测
            # detected_df_feature_labeled = detected_labeled_df[['efConstruction', 'M', 'efSearch', 'SIZE', 'q_SIZE', 'DIM', 'LID', 'Sum_K_MinDist', 'Sum_K_MaxDist', 'Sum_K_StdDist', 'q_K_MinRatio', 'q_K_MeanRatio','q_K_MaxRatio', 'q_K_StdRatio']]  #初始选择未标注数据时就可以用到
            ...
            distance_threshold = np.percentile(min_distance, 98)  # 更新距离阈值
            ...
            df_feature_query = df_feature_unlabeled[['efConstruction', 'M', 'efSearch', 'SIZE', 'q_SIZE', 'DIM', 'LID', 'Sum_K_MinDist', 'Sum_K_MaxDist', 'Sum_K_StdDist', 'q_K_MinRatio', 'q_K_MeanRatio','q_K_MaxRatio', 'q_K_StdRatio']]
            ...
            #'''
                ...
                for round_num in tqdm(range(selected_rounds), total = selected_rounds):
                    ...
                    # df_feature_labeled_selected = labeled_df_selected[['efConstruction', 'M', 'efSearch', 'SIZE', 'q_SIZE', 'DIM', 'LID', 'Sum_K_MinDist', 'Sum_K_MaxDist', 'Sum_K_StdDist', 'q_K_MinRatio', 'q_K_MeanRatio','q_K_MaxRatio', 'q_K_StdRatio']]
                    # 
                    # current_detected_df_feature_labeled = pd.concat([current_detected_df_feature_labeled, df_feature_labeled_selected], axis=0)
                    ...
                    current_selected_df_feature_labeled = current_train_df[['efConstruction', 'M', 'efSearch', 'SIZE', 'q_SIZE', 'DIM', 'LID', 'Sum_K_MinDist',
                    ...
                    min_distance = get_nn_dist(detected_labeled_feature_vectors, query_feature_vectors, 1)
                    mean_distance = np.mean(min_distance)
                    dist_flag = (mean_distance <= distance_threshold3)
