...
//            efs.push_back(i);
//        }
//        for (int i = 300; i < 700; i += 40) {
//            efs.push_back(i);
//        }
//        for (int i = 700; i < 1000; i += 60) {
//            efs.push_back(i);
//        }
//        for (int i = 1100; i < 2000; i += 100) {
//            efs.push_back(i);
//        }
//        for (int i = 2000; i <= 5000; i += 200) {
//            efs.push_back(i);
//        }
...
//            在训练数据上ef最小要是500,召回率需要达到0.998，在测试数据上对ef不需要有最小限制，最大限制仍为5000，glove是达到达到0.99
...
//        }
        ...
        for (size_t ef : efs) {
            ...
            results[ef] = {recall, count, time_us_per_query};
//            在训练数据上ef最小要是500,召回率需要达到0.995即可，在测试数据上对ef不需要有最小限制，最大限制仍为5000，glove是达到达到0.99
             if (ef >= 300 && (recall >= 0.995 || ef == 5000)) {
                break;
            }
...
#pragma omp critical
        ...
        for (const auto& pair : all_results) {
            int efS = pair.first;  // 获取 ef 值
            float rec = pair.second[0]; // 获取召回率
            size_t s_dc_counts = pair.second[1];
            float tpq = pair.second[2]; // 获取每查询的时间
